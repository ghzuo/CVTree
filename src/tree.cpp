...
/*******************************************************************/
Node::Node():name(""),id(0),length(NAN),parent(NULL),taxSize(0),
	     nleaf(0),nxleaf(0),unclassified(false),uploaded(false){
    children.reserve(N_FORKS);
};

Node::Node(size_t n):name(""),id(n),length(NAN),parent(NULL),taxSize(0),
                     nleaf(0),nxleaf(0),unclassified(false),uploaded(false){
    children.reserve(N_FORKS);
};

Node::Node(size_t n, const string& str):name(str),id(n),length(NAN),parent(NULL),taxSize(0),
                                        nleaf(0),nxleaf(0),unclassified(false),uploaded(false){
    children.reserve(N_FORKS);
};
...
void Node::chkLeafsName(vector<string>& defLeafs, vector<string>& undefLeafs, bool quite){
    vector<Node*> leafs;
    getLeafs(leafs);
    set<string> nameSet;
    for(auto &leaf : leafs){
